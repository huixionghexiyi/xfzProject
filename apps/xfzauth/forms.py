...
class RegisterForm(forms.Form, FormMixin):
    ...
    img_captcha = forms.CharField(max_length=4, min_length=4, required=True, error_messages={
                                  "required": "请输入图片验证码不能为空"})
    sms_captcha = forms.CharField(
        max_length=6, min_length=6, required=True, error_messages={"required": "短信验证码不能为空"})
    ...
    def clean(self):
        ...
        # 验证短信验证码(验证码为 123456，这里是假的使用短信验证码，节约短信次数)
